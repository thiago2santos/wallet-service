# Staging Environment for Wallet Service
# Smaller but representative environment for testing
...
terraform {
  ...
  # backend "s3" {
  #   bucket         = "wallet-service-terraform-state-staging"
...
module "networking" {
  ...
  enable_flow_logs        = false  # Disabled for cost savings in staging
}
...
module "database" {
  ...
  deletion_protection             = false  # Disabled for easier cleanup in staging
  skip_final_snapshot             = true   # Skip final snapshot in staging
  performance_insights_retention  = 7     # Shorter retention for cost savings
...
module "cache" {
  ...
  snapshot_retention_limit = 1  # Minimal snapshots for staging
...
module "messaging" {
  ...
  client_authentication = {
    tls = false  # Simplified for staging
  }
  encryption_in_transit = {
    client_broker = "PLAINTEXT"  # Simplified for staging
    in_cluster    = false
  }
  logging_info = {
    broker_logs = {
      cloudwatch_logs = {
        enabled   = true
        log_group = "/aws/msk/wallet-service-staging"
      }
      s3 = {
...
module "compute" {
  ...
  environment_variables = {
    ...
    KAFKA_BOOTSTRAP_SERVERS = module.messaging.bootstrap_brokers
  }
  ...
  # Logging
  log_group_name           = "/aws/ecs/wallet-service-staging"
