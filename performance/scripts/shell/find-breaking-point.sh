...
RESULTS_DIR="../../results/current/breaking-point-$(date +%Y%m%d_%H%M%S)"
mkdir -p "$RESULTS_DIR"

# Run the test and save results
k6 run --out json="$RESULTS_DIR/results.json" ../k6/continuous-wallet-stress.js 2>&1 | tee "$RESULTS_DIR/test-output.log"
